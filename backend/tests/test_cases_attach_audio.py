"""TDD T6.9 — POST /cases/{id}/modalities/audio → MinIO + fila `default`."""
...
# Conteúdo diferente para conflito de idempotência (mesmo formato WAV, 1 byte alterado).
AUDIO_ALT = AUDIO_SPEECH[:-1] + bytes([(AUDIO_SPEECH[-1] ^ 0x01)])
...
def test_attach_audio_requires_auth(client: TestClient) -> None:
    response = client.post(
        f"/cases/{uuid.uuid4()}/modalities/audio",
        headers={"Idempotency-Key": "a1"},
        files={"file": ("audio_speech.wav", AUDIO_SPEECH, "audio/wav")},
    )
    assert response.status_code == 401


def test_attach_audio_requires_idempotency_key(
    client: TestClient,
    operator_store: InMemoryOperatorStore,
) -> None:
    ...
    response = client.post(
        f"/cases/{case_id}/modalities/audio",
        headers=headers,
        files={"file": ("audio_speech.wav", AUDIO_SPEECH, "audio/wav")},
    )
...
def test_attach_audio_unknown_case_returns_404(
    client: TestClient,
    operator_store: InMemoryOperatorStore,
) -> None:
    _seed_medico(operator_store)
...
def test_attach_audio_stores_artifact_and_enqueues_default_queue(
    client: TestClient,
    operator_store: InMemoryOperatorStore,
    blob_store: InMemoryArtifactBlobStore,
    enqueuer: RecordingJobEnqueuer,
    ...
    response = client.post(
        f"/cases/{case_id}/modalities/audio",
        headers={**headers, "Idempotency-Key": "audio-attach-1"},
        files={"file": ("audio_speech.wav", AUDIO_SPEECH, "audio/wav")},
    )
    ...
    assert modalities["audio"]["status"] == "pending"
    assert modalities["audio"]["artifact_id"] is not None

    audio_arts = [a for a in body["artifacts"] if a["modality"] == "audio"]
    assert len(audio_arts) == 1
    art = audio_arts[0]
    digest = hashlib.sha256(AUDIO_SPEECH).hexdigest()
    assert art["content_sha256"] == digest
    assert art["object_key"].startswith(f"cases/{case_id}/audio/")
    assert blob_store.get(art["bucket"], art["object_key"]) == AUDIO_SPEECH

    assert len(enqueuer.jobs) == 1
    job = enqueuer.jobs[0]
    assert job["queue"] == DEFAULT_QUEUE_NAME
    assert job["queue"] != VIDEO_QUEUE_NAME
    assert job["payload"]["modality"] == "audio"
    assert job["payload"]["case_id"] == str(case_id)


def test_attach_audio_idempotent_replay(
    client: TestClient,
    operator_store: InMemoryOperatorStore,
    enqueuer: RecordingJobEnqueuer,
) -> None:
    ...
    headers = _auth_header(client)
    case_id = _create_case_with_vitals(client, headers)
    key_headers = {**headers, "Idempotency-Key": "audio-replay"}
...
def test_attach_audio_idempotency_conflict_different_content(
    client: TestClient,
    operator_store: InMemoryOperatorStore,
) -> None:
    _seed_medico(operator_store)
    headers = _auth_header(client)
    case_id = _create_case_with_vitals(client, headers)
    key_headers = {**headers, "Idempotency-Key": "audio-conflict"}
    ...
    conflict = client.post(
        f"/cases/{case_id}/modalities/audio",
        headers=key_headers,
        files={"file": ("audio_alt.wav", AUDIO_ALT, "audio/wav")},
    )
